...
const RestaurantHeader = ({navigation, btn, children, cart, onPress}) => {
  const cartList = useSelector((state) => state.restaurant.cart);
  ...
  return (
    <View style={styles().headerContainer}>
      <View style={[{...styles().headerRow}, btn ? {} : {paddingBottom: 20}]}>
        ...
        {cart ? (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate(routesNames.restaurantCart)}>
            <IconMD
              color={colors.black}
      ...
      {btn && (
        <PrimaryButton
          onPress={() => navigation.navigate(routesNames.restaurantInvoices)}
