...
const ReportItem = ({data, navigation, first, last}) => {
  const showReportDetails = () => {
    navigation.navigate(routesNames.labReportDetails, {
      data,
    });
  };
  return (
      ...
      <TouchableOpacity onPress={showReportDetails}>
        <View
          ...
          <Text style={{color: colors.orange, fontSize: 12, fontWeight: '500'}}>
            {I18nManager.isRTL
              ? converENDigitsToAr(
                  moment(data.DENOTED_DATE).format('YYYY/MM/DD'),
                )
              : moment(data.DENOTED_DATE).format('DD/MM/YYYY')}
          </Text>
        </View>
        <View style={{paddingLeft: 10, paddingTop: 10, paddingRight: 10}}>
          <Text
            style={{color: colors.darkGrey, fontSize: 14, textAlign: 'left'}}>
            {data.SERV_ENAME}
          </Text>
        </View>
        <View
