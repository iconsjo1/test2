...
const FilterModal = ({ categories, genders, filter, cancelFilter, show, setShow }) => {
    ...
    return (
        <Modal
            ...
            <View style={styles().modalContent}>
                ...
                <TouchableOpacity style={styles().inputStyle} onPress={() => setShowCategories(true)}>
                    <View style={styles().filterStyle}>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            ...
                            <Text style={{ color: colors.black, marginLeft: 10 }}>
                                {selectedCategory ?
                                    i18n.locale === 'ar' ?
                                        categories.find(el => el.CODE === selectedCategory).A_NAME
                                        :
                                        categories.find(el => el.CODE === selectedCategory).E_NAME
                                    :
                ...
                <TouchableOpacity style={styles().inputStyle} onPress={() => setShowGender(true)}>
                    <View style={styles().filterStyle}>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            {showCategories ?
                <FilterOverModal
                    data={categories.map(el => i18n.locale === 'ar' ? el.A_NAME : el.E_NAME).sort()}
                    onSelect={(cat) => {
                        const catEl = categories.find(el => el.A_NAME === cat || cat === el.E_NAME);
            ...
            {showGender ?
                <FilterOverModal
                    data={genders.map(el => i18n.locale === 'ar' ? el.A_NAME : el.E_NAME).sort()}
                    onSelect={(gender) => {
                        const genderEl = genders.find(el => el.A_NAME === gender || gender === el.E_NAME);
