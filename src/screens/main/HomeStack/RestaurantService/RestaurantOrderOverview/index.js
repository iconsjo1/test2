...
import { AppButton, LoadingWrapper, RestaurantHeader } from '../../../../../components';
...
import { rateOrder } from '../../../../../store/actions/main/restaurantActions';

const RestaurantOderOverview = ({ route, navigation }) => {
  const order = route?.params?.invoice;
  const oneOrder = route?.params?.oneOrder;
  const items = useSelector((state) => state.restaurant.cart);
  const paiedOrder = useSelector((state) => state.restaurant.orders);
...
export default RestaurantOderOverview;
